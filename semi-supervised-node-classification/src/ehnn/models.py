...
class MLP(nn.Module):
    """
    Code adapted from: CorrectAndSmooth in Equivariant Hypergraph Neural Networks, 2022,
     Qian Huang, Horace He, Abhay Singh, Ser-Nam Lim, Austin R. Benson
     Available from: https://github.com/CUAI/CorrectAndSmooth
     Article: https://arxiv.org/abs/2010.13993
     Accessed: 30 September 2023
    """
...
class EHNNTransformer(nn.Module):
    ...
    def forward(self, x, ehnn_cache, augmented_g=None):
        with torch.no_grad():
        ...
        x_v, x_e = self.layer1(x, ehnn_cache, augmented_g)
        x_v, x_e = self.dropout(x_v), self.dropout(x_e)
        x = (x_v, x_e)

        x = self.layer2(x, ehnn_cache, augmented_g)
        x = self.dropout(x)

        x_v, x_e = self.layer3(x, ehnn_cache, augmented_g)
        x_v, x_e = self.dropout(x_v), self.dropout(x_e)
        x = x_v, x_e
        x = self.layer4(x, ehnn_cache, augmented_g)
        x = self.dropout(x)
        learner_features = (x, x_e)
        x = self.output(x)
        return x, learner_features
