...
from .hypernetwork import PositionalEncoding, PositionalMLP
...
class SelfAttnV2E(nn.Module):
    ...
    def forward(self, x, ehnn_cache, augmented_g) -> [torch.Tensor, torch.Tensor]:
        """forward method
        :param x: [N, D]
        :param ehnn_cache:
        :return: [N, D], [|E|, D]
        ...
        # MLP1
        # Residual connection (shortcut) to help with vanishing gradient
        x = x + self.mlp1(self.norm1(x))  # [N, D] [Number of nodes, dimensionality of features]

        # 0 is global, 1 is local
        # generate query vectors layers
        ...
        # generate keys, compared against queries to generate attention scores
        k = self.k(x)
        k_0, k_1 = k.split(self.dim_qk, -1) # k generates two sets of keys so we split
        # split and stack each head
        k_0 = torch.stack(k_0.split(self.dim_qk_head, -1), 1)  # [N, H, D/H]
        k_1 = torch.stack(k_1.split(self.dim_qk_head, -1), 1)  # [N, H, D/H]
        # N = number of nodes, H = number of attention heads, D/H = dimension of each head (D is total dimension
        # of the keys/values)
        ...
        # Up to this point, we've just generated what we need for the attention mechanism,
        # so far we've only looked and node features

        # aggregation
        # either 0-overlap (global), or 1-overlap (local)
        # 0-overlap
        #  compute the dot product between q_0 and each of the keys in k_0 for each attention head,
        #  resulting in attention scores of shape [N, H]. (QK^T) / 1 sqrt(dk)
        logit0 = torch.einsum("hd,nhd->nh", q_0, k_0) / math.sqrt(
            self.dim_qk_head
        )  # [N, H]

        alpha0 = torch.softmax(logit0, dim=0)  # [N, H]
        # compute weighted sum of values v using attention weights
        att0 = torch.einsum("nh,nhd->hd", alpha0, v)  # [H, Dv/H] Dv/H = dimension of the value vectors for each head
        # concatenate results across all heads
        att0 = torch.cat(att0.unbind(-2), -1)  # [Dv,] Dv = concatenated results
        att0 = att0[None, :]  # [1, Dv]
        # 1-overlap node-to-node
        att1_v = torch.cat(v.unbind(-2), -1)  # [N, Dv] flatten value vectors across all heads
        ...
        # If augmented g is passed, apply it by retaining only preserved nodes in edges
        if augmented_g is not None:
            indices = indices.clone()
            indices[1] = indices[1] * augmented_g
        ...
        # normalisation
        t0 = self.norm2(att0) # (local, nodes only)
        t1_v = self.norm2(att1_v) # (global, nodes)
        t1_e = self.norm2(att1_e) # (global, edges)
        ...
        # final attention outputs for nodes (v) and edges (e)
        ...
        if self.max_l < len(edge_orders) and self.hyper_dropout == 0:
...
class SelfAttnE2V(nn.Module):
    ...
    def forward(self, x, ehnn_cache, augmented_g) -> [torch.Tensor, torch.Tensor]:
        ...
        incidence = ehnn_cache["incidence"]
        edge_orders = ehnn_cache["edge_orders"]
        indices_with_nodes = ehnn_cache["indices_with_nodes"] # not same as edge_order
        # MLP1
        x_v, x_e = x
        ...
        if self.max_k < len(edge_orders) and self.hyper_dropout == 0:
            # do not use this when hypernetwork is stochastic
        ...
        if augmented_g is not None:
            indices_with_nodes = indices_with_nodes.clone()
            head = indices_with_nodes[1][:len(augmented_g)] * augmented_g
            indices_with_nodes[1] = torch.cat((head, indices_with_nodes[1][len(augmented_g):]))
...
class TransformerV2E(nn.Module):
    ...
    def forward(self, x, ehnn_cache, augmented_g) -> [torch.Tensor, torch.Tensor]:
        h = self.ln(x)
        h_v, h_e = self.attn(h, ehnn_cache, augmented_g)
        x_v, x_e = (x + h_v, h_e)
...
class TransformerE2V(nn.Module):
    ...
    def forward(self, x, ehnn_cache, augmented_g) -> torch.Tensor:
        x_v, x_e = x
        h = (self.ln(x_v), self.ln(x_e))
        h = self.attn(h, ehnn_cache, augmented_g)
        x = x_v + h
        h = self.ffn(x)
        x = x + h
        return x
